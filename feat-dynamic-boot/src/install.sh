...
# The kernel image is versioned (/boot/vmlinuz-<ver>); the service needs a
# concrete --kernel to start from. mkuki watches /boot, so a later kernel swap
# still triggers a rebuild.
kernel="$(ls -1 /boot/vmlinuz-* 2>/dev/null | sort -V | tail -n1 || true)"
if [ -z "$kernel" ]; then
  echo "dynamic-boot: no /boot/vmlinuz-* kernel found" >&2
  exit 1
fi
...
{ "keys": [ { "path": "Machine/System/Services/mkuki-watch", "values": [
  { "name": "ImagePath", "type": "sz", "data": "/usr/bin/mkuki" },
