...
# and actually RUN them through the sysroot loader (the baked /lib64 interpreter
# path does not exist on the NixOS build box, so invoke ld.so explicitly).
#
#   nix develop ./_flake#cross -c sh pkgs/gcc/_cross/verify-cross.sh
